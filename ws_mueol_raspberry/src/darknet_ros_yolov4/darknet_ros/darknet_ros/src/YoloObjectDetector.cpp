      ...
      preempt_requested_(false)
{
  ...
  declare_parameter("subscribers.camera_reading.topic", std::string("camera_right"));
...
{
  // raspberry pi's low fps correction
  if((clock_ros->now() - time_last_process).seconds() < 5)
    return 0;
  time_last_process = clock_ros->now();
  // correction end
